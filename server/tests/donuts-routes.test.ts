...
describe("GET /me", () => {
  ...
  it('Should return a 500 if an error occurs', async () => {
    ...
    await request(app).get('/api/v1/donuts/me').expect(500)
...
describe('GET /:id', () => {
  it('Should return a donut', async () => {
    const res = await request(app).get('/api/v1/donuts/4').expect(200)
        .expect('Content-Type', /json/)
...
describe('DELETE /:id', () => {
  it('Should delete a donut', async () => {
    await request(app).delete("/api/v1/donuts/1")
        .set("Authorization", `Bearer 123`).expect(200)
  ...
  it('Should return a 400 if the ID is invalid', async () => {
    const res = await request(app).delete("/api/v1/donuts/abcde")
        .set("Authorization", `Bearer 123`).expect(400)
    expect(res.body.message).toMatch(/invalid/i)
  })

  it('Should return a 404 if the donut does not exist', async () => {
    const res = await request(app).delete("/api/v1/donuts/99")
        .set("Authorization", `Bearer 123`).expect(404)
    expect(res.body.message).toMatch(/does not exist/i)
  })
  ...
  it('Should return a 401 if the user id does not match', async () => {
    const res = await request(app).delete("/api/v1/donuts/4")
        .set("Authorization", `Bearer 123`).expect(401)
    expect(res.body.message).toMatch(/unauthorized/i)
  })

  it('Should return a 500 if an error occurs', async () => {
    vi.spyOn(db, "deleteDonut").mockRejectedValueOnce("YOU CANT TAKE HIM")
    await request(app).delete("/api/v1/donuts/2").set("Authorization", `Bearer 123`)
        .expect(500)
...
describe('POST /', () => {
  ...
  it('Should add a donut', async () => {
    const res = await request(app).post("/api/v1/donuts/")
        .set("Authorization", `Bearer 123`).send(donut).expect(200)
    expect(res.body).toStrictEqual({ id: 6, base: 1, glaze: 2, auth0_id: 'hello123' })
  })

  it('Should return a 401 if the user is unauthorized', async () => {
      const res = await request(app).post("/api/v1/donuts/")
          .send(donut).expect(401)
      expect(res.body.message).toMatch(/unauthorized/i)
  })

  it('Should return a 400 if not all details are added', async () => {
      const res = await request(app).post("/api/v1/donuts/")
          .set("Authorization", `Bearer 123`).send({glaze: 3}).expect(400)
      expect(res.body.message).toMatch(/missing/i)
  })

  it('Should return a 500 if an error occurs', async () => {
      vi.spyOn(db, "insertDonut").mockRejectedValueOnce("NO BAKING ALLOWED")
      await request(app).post("/api/v1/donuts/").set("Authorization", `Bearer 123`)
          .send(donut).expect(500)
  })
})
